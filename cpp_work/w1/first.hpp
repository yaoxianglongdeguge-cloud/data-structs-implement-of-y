...
//判断函数
bool empty() const{ return (row==0||column==0||data==nullptr); }//矩阵为空判断
...
//构造函数
matrix_:: matrix_(){}
matrix_::matrix_(int row_,int column_):data(nullptr),row(row_),column(column_){}
matrix_::matrix_(const matrix_ &a):data(nullptr),row(0),column(0)
{
...
matrix_::matrix_(std::initializer_list<std::initializer_list<double>>list):data(nullptr),row(0),column(0)
{
